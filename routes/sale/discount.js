...
router.post("/",roleGrant.grantAccess('createAny', 'discount'),function (req,res) {
    discountBal.addDiscount(req.body,function (data,err) {
        if(data.message=="error"){
            res.status(401).json(data);
        }else if(data.message=="success") {
            res.status(200).json(data);
